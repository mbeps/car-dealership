...
describe("config/env", () => {
  ...
  describe("validateEnv pure function usage", () => {
    beforeEach(() => {
      setValidRequiredEnv();
    });

    it("can validate pure input objects for client without touching process.env", async () => {
      const { validateEnv } = await loadEnvModule();

      const clientEnv = validateEnv(
        {
          NEXT_PUBLIC_SUPABASE_URL: "https://pure.supabase.co",
          NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: "pure-key",
        },
        false,
      );

      expect(clientEnv.NEXT_PUBLIC_SUPABASE_URL).toBe(
        "https://pure.supabase.co",
      );
      expect(clientEnv.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY).toBe("pure-key");
    });

    it("can validate pure input objects for server without touching process.env", async () => {
      const { validateEnv } = await loadEnvModule();
      ...
      const serverEnv = validateEnv(
        {
          NEXT_PUBLIC_SUPABASE_URL: "https://pure.supabase.co",
          NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: "pure-key",
          SUPABASE_SECRET_KEY: "pure-secret",
          ARCJET_KEY: "pure-arcjet",
        },
        true,
      );

      expect(serverEnv.SUPABASE_SECRET_KEY).toBe("pure-secret");
      expect(serverEnv.ARCJET_KEY).toBe("pure-arcjet");
    });
  });
});
