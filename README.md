...
- [**Shadcn UI**](https://ui.shadcn.com/): Shadcn UI provides beautifully designed, accessible components built on top of Base UI and Tailwind CSS. These components are customizable and integrate seamlessly into modern applications. The primitives live in `components/ui/` and are regenerated via the shadcn CLI.
...
- `SUPABASE_JWT_SECRET`: Optional JWT secret used by Supabase server-side auth flows.
- `NEXT_PUBLIC_SITE_URL`: The canonical site URL used when generating absolute links. Useful in production deployments.
- `NEXT_PUBLIC_VERCEL_URL`: Vercel's autogenerated deployment URL. Used as a fallback when `NEXT_PUBLIC_SITE_URL` is not set.
- `NODE_ENV`: The application runtime environment (`development`, `test`, or `production`).
